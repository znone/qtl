...
namespace detail
{
	...
	struct data_type<M T::*>
	{
		typedef M type;
	};

	template<class Derived, class Base, typename M, typename = std::enable_if<std::is_base_of<Base, Derived>::value>::type>
	inline M Derived::* member_cast(M Base::* m)
	{
		return static_cast<M Derived::*>(m);
	};
}

template<class T, typename... Members>
inline struct bind_struct_t<T, typename detail::data_type<Members>::type...> bind_struct(Members... members)
{
	return bind_struct_t<T, typename detail::data_type<Members>::type...>(detail::member_cast<T>(members)...);
}

template<class T, typename... Members>
inline struct bind_struct_t<T, typename detail::data_type<Members>::type...> bind_struct(T& obj, Members... members)
{
	return bind_struct_t<T, typename detail::data_type<Members>::type...>(obj, detail::member_cast<T>(members)...);
}
